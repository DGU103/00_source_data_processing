...
Function Validate_Tag{
    ...
    foreach($ELEMENT in $NAMING_TEMPLATE.Elements.ChildNodes | Sort-Object {$_.sortorder}){
        ...
        if($tag_validation_string -match "^"+$regexp) {
...
Function Import-MTR-XLSX{
    param($Path, [int]$StartRow=1,[int]$EndRow=100000 )
...
if($UpdateRegexConfigurations -eq 'Y' -or $UpdateRegexConfigurations -eq 'y'){
    ECHO "Updating FULL REGEXP configuration file"
    .\build_full_regex_config.ps1

    ECHO "Updating LIGHT REGEXP configuration file"
    .\build_light_regex_config.ps1
}

if($Path -match "xlsx$"){
 $Tag_data_table = Import-MTR-XLSX -Path $Path -StartRow 1 -EndRow 100000
...
$light_regex_csv = Import-Csv -Path ".\Light_regex.csv" -Delimiter ";"
$full_regex_csv = Import-Csv -Path ".\Full_regex.csv" -Delimiter ";"
...
if($Tag_data_table[5].Tag_Number.split('-')[0] -match 'BH|BE'){
...
$validation_report = New-Object Validation_Message[] 100000
$vri = 0
...
foreach($row in $Tag_data_table){
    ...
    IF($tag -match '\.[0-9]{1,2}[A-Z]?$'){
        $PARENT_TAG_source = $tag.Split('.')[0]

        if(-not $Tag_table[$PARENT_TAG_source] ){
                $vm = New-Object -TypeName Validation_Message
                $vm.Tag_number = $tag 
                $vm.Tag_class = $tag_class
                $vm.Tag_description = $tag_description
                ...
                $vm.Validation_Status = "Not Valid"
                $vm.Error_code = "#0001"
                $vm.Validation_message ="Parent Tag "+ $PARENT_TAG.Text +" not found in current register"
                $vm.Service_message = "Parent Tag"
                $vm.Tag_format = "Instrumentation"
...
#>   
    ...
    IF($tag -match "[A-Z]{5}-(FCV|ICD|LCV|PCV|PSV|SOV|TCV)-[0-9]{6}[A-Z]"){
        ...
        if ($Tag_table[$targetTag])
        {
            ...
            $vm.Tag_format = $validation_report[$vri-1].Tag_format
...
$validation_report | where {$_.Tag_number} | Export-Csv -Path $csv_path -NoTypeInformation


# --- AIM-A Export --- #
$aim_export_path = [System.IO.Path]::Combine("\\als.local\NOC\Data\Appli\DigitalAsset\MP\RUYA_data\RegisterGateway\source data\data load\MP\MTR\", 'MTR_' + $package_code+ "_Tag_Load_to_AIM-A.csv")
$validation_report | where {$_.Tag_number -and $_.Validation_Status -eq "Valid"} |select ("Tag_number", "Tag_class", "Tag_description", "Action", "Status") -Unique | Export-Csv -Path $aim_export_path -NoTypeInformation
...
$csv | select ("Tag_number", "Validation_Status") -Unique | Export-Excel $xlsx_path -WorksheetName "Sheet2" -FreezeTopRow -AutoFilter -AutoNameRange -BoldTopRow -NoNumberConversion *
